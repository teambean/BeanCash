    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(wallet.strWalletFile) || bitdb.mapFileUseCount[wallet.strWalletFile] == 0)
            {
                ...
                LogPrintf("Issuing Log Sequence Number reset for backup file portability.\n");
...
bool DumpWallet(CWallet* pwallet, const string& strDest)
{
  ...
  while (true)
  {
      ...
      // produce output
      file << strprintf("# Wallet dump created by Bean Cash Core %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
...
bool ImportWallet(CWallet *pwallet, const string& strLocation)
{
   ...
   while (true)
   {
      ...
      // rescan block chain looking for Beans from new keys
