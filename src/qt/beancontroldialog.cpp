...
using namespace std;
QList<qint64> BeanControlDialog::payAmounts;
CBeanControl* BeanControlDialog::beanControl = new CBeanControl();

BeanControlDialog::BeanControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::BeanControlDialog),
    model(0)
{
    ui->setupUi(this);
...
BeanControlDialog::~BeanControlDialog()
{
    delete ui;
}

void BeanControlDialog::setModel(WalletModel *model)
{
    this->model = model;
...
QString BeanControlDialog::strPad(QString s, int nPadLength, QString sPadding)
{
...
// ok button
void BeanControlDialog::buttonBoxClicked(QAbstractButton* button)
{
...
// (un)select all
void BeanControlDialog::buttonSelectAllClicked()
{
...
// context menu
void BeanControlDialog::showMenu(const QPoint &point)
{
...
void BeanControlDialog::copyAddress()
{
...
void BeanControlDialog::copyTransactionHash()
{
...
void BeanControlDialog::headerSectionClicked(int logicalIndex)
{
...
// toggle tree mode
void BeanControlDialog::radioTreeMode(bool checked)
{
    if (checked && model)
        updateView();
}
...
// toggle list mode
void BeanControlDialog::radioListMode(bool checked)
{
    if (checked && model)
        updateView();
}
...
void BeanControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            beanControl->UnSelect(outpt);
        else if (item->isDisabled()) // locked (this happens if "check all" through parent node)
            item->setCheckState(COLUMN_CHECKBOX, Qt::Unchecked);
        else
            beanControl->Select(outpt);
...
QString BeanControlDialog::getPriorityLabel(double dPriority)
{
...
void BeanControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    foreach(const qint64 &amount, BeanControlDialog::payAmounts)
    {
        nPayAmount += amount;
    ...
    {
        // Bytes
        nBytes = nBytesInputs + ((BeanControlDialog::payAmounts.size() > 0 ? BeanControlDialog::payAmounts.size() + 1 : 2) * 34) + 10; // always assume +1 output for change here
        
        // Priority
        dPriority = dPriorityInputs / nBytes;
        sPriorityLabel = BeanControlDialog::getPriorityLabel(dPriority);
    ...
    int nDisplayUnit = BitbeanUnits::BitB;
    if (model && model->getOptionsModel())
        nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
            
    QLabel *l1 = dialog->findChild<QLabel *>("labelBeanControlQuantity");
    QLabel *l2 = dialog->findChild<QLabel *>("labelBeanControlAmount");
    QLabel *l3 = dialog->findChild<QLabel *>("labelBeanControlFee");
    QLabel *l4 = dialog->findChild<QLabel *>("labelBeanControlAfterFee");
    QLabel *l5 = dialog->findChild<QLabel *>("labelBeanControlBytes");
    QLabel *l6 = dialog->findChild<QLabel *>("labelBeanControlPriority");
    QLabel *l7 = dialog->findChild<QLabel *>("labelBeanControlLowOutput");
    QLabel *l8 = dialog->findChild<QLabel *>("labelBeanControlChange");
    
    // enable/disable "low output" and "change"
    dialog->findChild<QLabel *>("labelBeanControlLowOutputText")->setEnabled(nPayAmount > 0);
    dialog->findChild<QLabel *>("labelBeanControlLowOutput")    ->setEnabled(nPayAmount > 0);
    dialog->findChild<QLabel *>("labelBeanControlChangeText")   ->setEnabled(nPayAmount > 0);
    dialog->findChild<QLabel *>("labelBeanControlChange")       ->setEnabled(nPayAmount > 0);
    
    // stats
    l1->setText(QString::number(nQuantity));                                 // Quantity        
    l2->setText(BitbeanUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BitbeanUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BitbeanUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(BitbeanUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    
    // turn labels "red"
    l5->setStyleSheet((nBytes >= 10000) ? "color:red;" : "");               // Bytes >= 10000
    l6->setStyleSheet((dPriority <= 576000) ? "color:red;" : "");         // Priority < "medium"
    l7->setStyleSheet((fLowOutput) ? "color:red;" : "");                    // Low Output = "yes"
    l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01BitB
        
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)).arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)).arg(BitbeanUnits::formatWithUnit(nDisplayUnit, CENT)));
    dialog->findChild<QLabel *>("labelBeanControlBytesText")    ->setToolTip(l5->toolTip());
    dialog->findChild<QLabel *>("labelBeanControlPriorityText") ->setToolTip(l6->toolTip());
    dialog->findChild<QLabel *>("labelBeanControlLowOutputText")->setToolTip(l7->toolTip());
    dialog->findChild<QLabel *>("labelBeanControlChangeText")   ->setToolTip(l8->toolTip());
   
    // Insufficient funds
    QLabel *label = dialog->findChild<QLabel *>("labelBeanControlInsuffFunds");
    if (label)
        label->setVisible(nChange < 0);
}

void BeanControlDialog::updateView()
{
    ...
    for (std::pair<QString, vector<COutput>> beans : mapBeans)
    {
        ...
        for (const COutput& out : beans.second)
        {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BitbeanUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            ...
            itemOutput->setText(COLUMN_PRIORITY, BeanControlDialog::getPriorityLabel(dPriority));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BitbeanUnits::format(nDisplayUnit, nSum));
