...
int getBlockHashrate(int height)
{
    ...
    return (boost::int64_t)(((double)getBlockHardness(height) * pow(1.0, 32)) / timePerBlock);
}
...
std::string getBlockHash(int Height)
{
    ...
    if(Height < 0) { return "000009d2f828234d65299216e258242a4ea75d1b8d8a71d076377145068f08de"; }
    int desiredheight;
...
double getTxTotalValue(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        buffer = value + convertBeans(txout.nValue);
        value = buffer;
    }
...
double convertBeans(int64_t amount)
{
...
std::string getOutputs(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        CBitbeanAddress addressSource(source);
        std::string lol7 = addressSource.ToString();
        double buffer = convertBeans(txout.nValue);
        std::string amount = boost::to_string(buffer);
        str.append(lol7);
        str.append(": ");
        str.append(amount);
        str.append(" Bean Cash");
        str.append("\n");
    }
...
std::string getInputs(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        double buffer = convertBeans(getInputValue(wtxPrev, target));
        std::string amount = boost::to_string(buffer);
        str.append(lol6);
        str.append(": ");
        str.append(amount);
...
double getTxFees(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        ssTx << wtxPrev;
        const CScript target = wtxPrev.vout[vin.prevout.n].scriptPubKey;
        buffer0 = value0 + convertBeans(getInputValue(wtxPrev, target));
        value0 = buffer0;
    }
...
void BlockBrowser::updateExplorer(bool block)
{
    ...
    if(block == false) {
        ...
        ui->valueBox->setText(QValue + " Bean Cash");
        ui->txID->setText(QID);
        ui->outputBox->setText(QOutputs);
        ui->inputBox->setText(QInputs);
        ui->feesBox->setText(QFees + " Bean Cash");
    }
}
