...
const int BITBEAN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITBEAN_IPC_PREFIX("beancash:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BITBEAN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BITBEAN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click beancash: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start beancash: click-to-pay handler");
    else
...
{
    // clicking on beancash: URLs creates FileOpen events on the Mac:
