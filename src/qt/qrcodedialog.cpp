...
{
    QString ret = QString("Beancash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BitB unit input in lnReqAmount, we generate the URI with BitB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitbeanUnits::format(BitbeanUnits::BitB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
