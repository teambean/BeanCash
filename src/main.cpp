...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fCheckInputs, bool* pfMissingInputs)
{
    if (pfMissingInputs)
    ...
    if (tx.IsBeanBase())
        return tx.DoS(100, error("AcceptToMemoryPool : beanbase as individual tx"));

    // Beansprout is also only valid in a block, not as a loose transaction
    if (tx.IsBeanStake())
        return tx.DoS(100, error("AcceptToMemoryPool : beansprout as individual tx"));
...
{
    return ::AcceptToMemoryPool(mempool, *this, true, NULL);
}
...
// minimum proof-of-bean required was nBase
//
...
// Total bean age spent in transaction, in the unit of bean-days.
...
bool CTransaction::GetBeanAge(CTxDB& txdb, uint64_t& nBeanAge) const
{
    ...
    if (fDebug && GetBoolArg("-printbeanage"))
    {
        LogPrint("beanage", "bean age bnBeanDay=%s\n", bnBeanDay.ToString());
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        // Beanbase output should be empty if proof-of-bean block
        if (vtx[0].vout.size() != 1 || !vtx[0].vout[0].IsEmpty())
            return DoS(100, error("CheckBlock() : beanbase output not empty for proof-of-bean block"));
        ...
        if (fCheckSig && !CheckBlockSignature())
            return DoS(100, error("CheckBlock() : bad proof-of-bean block signature"));
    }
...
bool CBlock::AcceptBlock()
{
    ...
    // Check proof-of-work or proof-of-bean
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))
        return DoS(100, error("AcceptBlock() : incorrect %s", IsProofOfWork() ? "proof-of-work" : "proof-of-bean"));
    ...
    if (IsProofOfStake())
    {
        ...
        {
            LogPrintf("WARNING: AcceptBlock(): check proof-of-bean failed for block %s\n", hash.ToString());
...
// Attempt to generate suitable proof-of-bean
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    // if we are trying to sign
    //    something except proof-of-bean block template
    if (!vtx[0].vout[0].IsEmpty())
        return false;

    // if we are trying to sign
    //    a complete proof-of-bean block
    if (IsProofOfStake())
        return true;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    {
        string strMessage = _("Error: Storage space is low! Shutting down...");
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // download node to accept as orphan (proof-of-bean
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION || pfrom->nVersion == 0)
        {
    ...
    else if (strCommand == "inv")
    {
        ...
        {
            pfrom->Misbehaving(20);
            return error("message inventory size() = %u", vInv.size());
        }
    ...
    if (lockMain)
    {
       ...
       if (pto->fPingQueued) {
        ...
        if (pto->nPingNonceSent == 0 && pto->nPingUsecStart + PING_INTERVAL * 1000000 < GetTimeMicros()) {
            // Ping automatically sent as latency probe and keepalive.
            pingSend = true;
        }
        if (pingSend) {
            uint64_t nonce = 0;
