...
inline int LogPrint(const char* category, const char* pszFormat, ...)
{
    if (category != NULL)
        {
            if (!fDebug) return 0;
...
string FormatMoney(int64_t n, bool fPlus)
{
    ...
    int64_t quotient = n_abs/bean;
    int64_t remainder = n_abs%bean;
...
bool ParseMoney(const char* pszIn, int64_t& nRet)
{
    ...
    if (strWhole.size() > 10) // guard against 63 bit overflow
        return false;
    if (nUnits < 0 || nUnits > bean)
        return false;
    int64_t nWhole = atoi64(strWhole);
    int64_t nValue = nWhole*bean + nUnits;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitBean
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitBean
    // Mac: ~/Library/Application Support/BitBean
    // Unix: ~/.BitBean
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitBean";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Beancash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Beancashd.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.bitbean.it/wiki/BIP_0014)
...
// pthread_setupname_np is XCode 10.6 and later
...
// Increase file descriptor limit to the argument amount and return actual limit
int RaiseFileDescriptorLimit(int nMinFD) {
    ...
    return nMinFD; // getrlimit failied, assume it's fine
#endif
}
