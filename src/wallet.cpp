...
bool CWallet::SelectBeansMinConfByBeanAge(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet) const
{
    ...
    if (denom != nGCD)
    {
        // All Bean outputs with zero value will always be added by the dynamic programming routine
        // So we collect them into bundles of value denomination
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& strFailReason, const CBeanControl* beanControl)
{
    int64_t nValue = 0;
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-beancash-address
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& strFailReason, const CBeanControl* beanControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, beanControl);
}
...
bool CWallet::CreateBeanStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    for (std::pair<const CWalletTx*, unsigned int> pbean : setBeans)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBest; n++)
        {
            ...
            {
                // Found a kernel
                LogPrint("Bean Sprout", "CreateBeanSprout : kernel found\n");
                ...
                {
                    LogPrint("Bean Sprout", "CreateBeanSprout : failed to parse kernel\n");
                    break;
                }
                    LogPrint("Bean Sprout", "CreateBeanSprout : parsed kernel type=%d\n", whichType);
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
                {
                    LogPrint("Bean Sprout", "CreateBeanSprout : no support for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    {
                        LogPrint("Bean Sprout", "CreateBeanSprout : failed to get key for kernel type=%d\n", whichType);
                ...
                {
                    LogPrint("Bean Sprout", "CreateBeanSprout : invalid key for kernel type=%d\n", whichType);
                    break; // keys mismatch
                }
                ...
                LogPrint("Bean Sprout", "CreateBeanSprout : added kernel type=%d\n", whichType);
                fKernelFound = true;
                break;
            }
        }
...
DBErrors CWallet::ZapWalletTx(std::vector<CWalletTx>& vWtx)
{
	...
	if (nZapWalletTxRet == DB_NEED_REWRITE)
	{
		if (CDB::Rewrite(strWalletFile, "\x04pool"))
		{
			LOCK(cs_wallet);
			setKeyPool.clear();
			// Can't top-up keypool as wallet is locked.
			// User will be prompted to unlock wallet on next operation that needs a new key.		
		}	
	}
...
void CWallet::FixSpentBeans(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    for (CWalletTx* pbean : vBeans)
    {
        ...
        for (unsigned int n=0; n < pbean->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentBeans found lost bean %s TC %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentBeans found spent bean %s TC %s[%d], %s\n",
                    FormatMoney(pbean->vout[n].nValue).c_str(), pbean->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pbean->vout[n].nValue;
                if (!fCheckOnly)
